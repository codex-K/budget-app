...
const ChartTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl shadow-lg px-3 py-2 text-sm">
      ...
      <p style={{ color: d.payload?.fill || '#6366f1' }}>${fmt(d.value)}</p>
...
export default function Dashboard() {
  ...
  const myNet = myTotalIncome - myTotalExp - myShareOfBills
  ...
  const chartData = Object.entries(categoryTotals)
    ...
    .map(([category, value], i) => ({
      category, value: parseFloat(value.toFixed(2)),
          ...
          <div className="space-y-6">
            ...
            {recurringNotice !== null && recurringNotice > 0 && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 flex items-center justify-between text-sm">
                <span className="text-emerald-700">✅ <strong>{recurringNotice} recurring item{recurringNotice > 1 ? 's' : ''}</strong> automatically added for {monthName}</span>
            ...
            {recurringNotice === -1 && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700">
                ⚠️ Some recurring items may not have loaded. Try refreshing.
              </div>
            )}
            ...
            {budgetWarnings.length > 0 && (
              <div className="space-y-2">
                {budgetWarnings.map(w => (
                  <div key={w.id} className={`rounded-xl px-4 py-3 flex items-center justify-between text-sm ${w.pct >= 100 ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>
                    <span>{w.pct >= 100 ? '🚨' : '⚠️'} <strong>{w.category}</strong> — {w.pct >= 100 ? 'over budget!' : `${w.pct.toFixed(0)}% used`}</span>
            ...
            {isEmpty ? (
              <div className="space-y-3">
                <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-6">
                  ...
                  <p className="text-indigo-600 text-sm">No data entered yet. Get started below.</p>
                </div>
                {[
                  { to: '/income', icon: '💵', label: 'Add your income', desc: 'Salary and other income sources', cta: 'Add Income' },
                  { to: '/expenses', icon: '🧾', label: 'Add personal expenses', desc: 'Your individual spending', cta: 'Add Expenses' },
                  { to: '/shared', icon: '🏠', label: 'Add shared bills', desc: 'Bills split with your partner', cta: 'Add Bills' },
                  { to: '/budget', icon: '📊', label: 'Set budget limits', desc: 'Get warnings before overspending', cta: 'Set Limits' },
                  { to: '/savings', icon: '🎯', label: 'Create a savings goal', desc: 'Set targets to work towards', cta: 'Add Goal' },
                  ...
                  <Link key={to} to={to}
                    ...
                    <span className="text-xs text-indigo-500 font-medium bg-indigo-50 px-3 py-1.5 rounded-lg group-hover:bg-indigo-100 transition flex-shrink-0 ml-4">{cta} →</span>
                  </Link>
                ))}
              </div>
            ) : (
                ...
                <div className={`grid gap-4 ${partner ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'}`}>
                  ...
                  {partner && (
                    <div className={`rounded-2xl p-5 ${isPos(partnerNet) ? 'bg-gradient-to-br from-sky-500 to-blue-500' : 'bg-gradient-to-br from-orange-500 to-red-500'} text-white`}>
                      ...
                      <p className="text-white/80 text-sm">Monthly surplus / deficit</p>
                      <p className="text-3xl font-bold mt-1">{isPos(partnerNet) ? '+' : '-'}${fmt(Math.abs(partnerNet))}</p>
                      <div className="mt-3 space-y-1 text-xs text-white/70">
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
                        {[{ key: 'donut', label: '◎' }, { key: 'pie', label: '●' }, { key: 'bar', label: '▦' }].map(({ key, label }) => (
                          ...
                          <ResponsiveContainer width="100%" height={200}>
                            <PieChart>
                              <Pie data={chartData} dataKey="value" nameKey="category" cx="50%" cy="50%"
                                innerRadius={chartView === 'donut' ? 55 : 0} outerRadius={90}
